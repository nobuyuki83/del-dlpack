...
pub unsafe fn is_tensor_c_contiguous(t: &Tensor) -> bool {
    unsafe { is_c_contiguous(t.shape, t.strides, t.ndim) }
}
...
pub unsafe fn slice_from_tensor<'a, T>(t: &dlpack::Tensor) -> Option<&'a [T]> {
    ...
    let sh = unsafe { slice::from_raw_parts(t.shape, t.ndim as usize) }; // ← ここでスライス化
    let len = sh
        .iter()
    ...
    let s = if t.data.is_null() {
        ...
        unsafe { slice::from_raw_parts(t.data as *const T, len) }
    };
    Some(s)
}
...
pub unsafe fn slice_from_tensor_mut<'a, T>(t: &dlpack::Tensor) -> Option<&'a mut [T]> {
    assert!(unsafe { is_c_contiguous(t.shape, t.strides, t.ndim) });
    if t.shape.is_null() || t.ndim < 0 {
        return None;
    }
    use std::slice::{from_raw_parts, from_raw_parts_mut};
    let sh = unsafe { from_raw_parts(t.shape, t.ndim as usize) }; // ← ここでスライス化
    let len = sh
        .iter()
        .try_fold(1usize, |acc, &d| acc.checked_mul(d as usize))?;
    let s = if t.data.is_null() {
        assert_eq!(len, 0, "null なら len==0 である必要があります");
        unsafe { from_raw_parts_mut(std::ptr::NonNull::<T>::dangling().as_ptr(), 0) }
    } else {
        // アラインメント自己診断（本番では assert を外してもOK）
        assert_eq!(
        ...
        unsafe { from_raw_parts_mut(t.data as *mut T, len) }
    };
    Some(s)
}
...
unsafe extern "C" fn capsule_destructor(capsule: *mut pyo3::ffi::PyObject) {
    ...
    let ptr = unsafe { pyo3::ffi::PyCapsule_GetPointer(capsule, name.as_ptr()) };
    if ptr.is_null() {
        unsafe { pyo3::ffi::PyErr_Clear() }; // ← これが肝心（例外を外へ漏らさない）
        return;
    }
    let m = ptr as *mut dlpack::ManagedTensor;
    if !m.is_null() {
        let deleter = unsafe { (*m).deleter };
        deleter(m);
    }
}

pub fn make_capsule_from_vec<T>(
    py: Python,
    shape_vec: Vec<i64>,
    mut data: Vec<T>,
) -> pyo3::Py<PyAny>
...
#[cfg(feature = "cuda")]
pub fn make_capsule_from_cuvec<T: ToDataTypeCode>(
    py: Python,
    device_id: i32,
    shape: Vec<i64>,
